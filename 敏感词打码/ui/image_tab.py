...
from engine.img_engine import ImgEngine
...
class ImgTab(QWidget):
    ...
    def _build(self):
        ...
        self.btn_import = QPushButton("📥  导入图片文件")
        ...
        g1 = QGroupBox("📋  图片列表")
        g1.setStyleSheet("QGroupBox{font-size:12px;}")
        l1 = QVBoxLayout(g1)
        ...
        r2.addSpacing(8)
        self.btn_go = QPushButton("🚀  批量导出打码图片")
    ...
    def _wire(self):
        self.btn_import.clicked.connect(lambda: self._add_files(
            QFileDialog.getOpenFileNames(self, "选择图片", "",
            "图片 (*.jpg *.jpeg *.png *.bmp *.tiff *.tif)")[0]))
    ...
    def _add_files(self, files):
        ...
        self.lbl_cnt.setText(f"已导入 {len(self.files)} 张图片")
    ...
    def _clear(self):
        ...
        self.lbl_cnt.setText("拖拽图片到此处或点击导入"); self.preview.clear()
    ...
    def _preview(self):
        ...
        if not sel: return QMessageBox.information(self, "提示", "请选中一张图片")
        idx = self.list_w.row(sel[0])
        if idx >= len(self.files): return
        cfg = self.rules.config()
        eng = ImgEngine(Detector(cfg["rules"], cfg["keywords"]), MODES[self.combo.currentText()])
        self.preview.show_orig(eng.preview(self.files[idx], False))
        self.preview.show_mask(eng.preview(self.files[idx], True))

    def _run(self):
        if not self.files: return QMessageBox.warning(self, "警告", "请先导入图片")
        ...
        self.worker = ImgWorker(self.files, out, MODES[self.combo.currentText()], cfg)
